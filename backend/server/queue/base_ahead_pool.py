...
Subclasses (DownloadAheadPool, FileTaskParsePool) implement _loop() for their
specific processing logic and _unload_models() for cleanup on stop.
"""
...
class BaseAheadPool(ABC):
    """Base class for background daemon pools.

    Current subclasses (the NAS/remote pre-processing chain):
    - DownloadAheadPool: fetches remote (WebDAV) originals for pending file_tasks
    - FileTaskParsePool: parses downloaded/local files before the AI phases
