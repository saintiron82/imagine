...
class ParseAheadPool(BaseAheadPool):
    ...
    def _run_vv_buffer(self) -> bool:
        """Generate VV (SigLIP2) vectors for parsed jobs missing VV embeddings.

        Only runs when _processing_mode == 'parse_vv'.
        Uses thumbnails (already generated by Phase P) as input.

        Returns True if at least one VV vector was generated.
        """
        cursor = self.db.conn.cursor()
        cursor.execute("""
            SELECT jq.id, jq.file_id, f.thumbnail_url
            FROM job_queue jq JOIN files f ON f.id = jq.file_id
            WHERE jq.parse_status = 'parsed' AND jq.status = 'pending'
              AND f.thumbnail_url IS NOT NULL AND f.thumbnail_url != ''
              AND NOT EXISTS (SELECT 1 FROM vec_files WHERE file_id = jq.file_id)
        ...
        # Lazy-load SigLIP2 encoder
        if not self._vv_encoder:
            try:
                from backend.vector.siglip2_encoder import SigLIP2Encoder
                self._vv_encoder = SigLIP2Encoder()
                logger.info("ParseAheadPool: SigLIP2 encoder loaded for VV processing")
            except Exception as e:
                logger.error(f"ParseAheadPool: Failed to load SigLIP2: {e}")
                return False

        vv_count = 0
        for job_id, file_id, thumb_url in jobs:
            try:
                # Resolve thumbnail path
                from pathlib import Path as _Path
                thumb_path = _Path(thumb_url) if thumb_url else None
                if not thumb_path or not thumb_path.exists():
                    # Try relative to project output
                    thumb_path = _Path("output/thumbnails") / _Path(thumb_url).name
                    if not thumb_path.exists():
                        continue

                import numpy as np
                embedding = self._vv_encoder.encode_image(str(thumb_path))
                if embedding is not None:
                    embedding_list = np.array(embedding).astype(np.float32).tolist()
                    ...
                    vv_count += 1
                    logger.debug(f"VV generated for file_id={file_id}")
            except Exception as e:
                logger.warning(f"VV generation failed for file_id={file_id}: {e}")

        if vv_count > 0:
            self.db.conn.commit()
            logger.info(f"ParseAheadPool: generated {vv_count} VV vectors")

        return vv_count > 0

    def _loop(self):
        """Main loop: dynamic mode (parse_only or parse_vv).

        - parse_only: Phase P only (zero GPU)
        - parse_vv: Phase P + VV embedding (SigLIP2, ~1.5GB GPU)
        """
        logger.info(f"ParseAheadPool loop started (mode={self._processing_mode})")
        ...
        try:
            while self._running:
                try:
                    ...
                    # parse_vv mode: also generate VV embeddings
                    if self._processing_mode == "parse_vv":
                        self._run_vv_buffer()
