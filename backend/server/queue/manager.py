...
class JobQueueManager:
    ...
    def update_phase(self, job_id: int, phase: str, value: bool = True):
        ...
        # Map phase names to actual DB keys (vv and mv are tracked separately)
        db_key = {"mc": "vision"}.get(phase, phase)
        pc[db_key] = value
        now = _utcnow_sql()
        ...
        for folder_path, files in file_groups.items():
            ...
            for chunk_idx, chunk in enumerate(chunks):
                ...
                for fid, fpath in chunk:
                    ...
                    phase_completed = json.dumps({
                        "parse": True,
                        "mc": data.get("has_mc", False),
                        "vv": data.get("has_vv", False),
                        "mv": data.get("has_mv", False),
                    })
                    file_ready = 0 if fpath.startswith("webdav://") else 1
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if parsed_count > 0:
                ...
                buffer_need_vv = brow[1] or 0
                buffer_need_mv = brow[2] or 0
