...
def download_file(
    ...
    # Remote URIs (webdav://) — original file lives on NAS, not on server disk.
    # Fall back to thumbnail if available (generated by WebDAV browse).
    if file_path_str and file_path_str.startswith(("webdav://", "http://", "https://")):
        thumb_url = row[1]
        if thumb_url and Path(thumb_url).exists():
            return FileResponse(
                ...
                headers={"X-Imagine-Fallback": "thumbnail"},
            )
        raise HTTPException(
            status_code=404,
            detail="Remote file (WebDAV/NAS) — original not on server disk and no thumbnail available",
        )
