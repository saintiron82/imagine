...
class WorkerDaemon:
    ...
    def _resolve_file(self, job: dict) -> str:
        ...
        For non-local paths (webdav://, http://, etc.), the original file
        lives on a remote server (NAS) that the worker can't access directly.
        In this case, always try thumbnail first (generated by WebDAV browse).
        """
        file_path = job["file_path"]
