...
function App() {
      ...
      try {
        ...
        if (data && data.success !== false) {
          setServerQueueStats({
            ...
            complete_files: data.complete_files ?? data.completed ?? 0,
  ...
  const [retryResetStuck, setRetryResetStuck] = useState(false);
                ...
                <div className="absolute right-0 top-full mt-1 z-50 bg-gray-800 border border-gray-600 rounded-lg shadow-xl py-1 min-w-[180px]">
                  ...
                  {isAdmin && (
                    <>
                      ...
                      <label className="flex items-center gap-2 px-4 py-1.5 text-[11px] text-gray-400 cursor-pointer hover:text-gray-300 select-none">
                        <input
                          type="checkbox"
                          checked={retryResetStuck}
                          onChange={(e) => setRetryResetStuck(e.target.checked)}
                          className="w-3 h-3 rounded border-gray-600 bg-gray-800 text-orange-500 focus:ring-0 focus:ring-offset-0"
                        />
