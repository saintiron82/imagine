...
현재 3축:
  VV:  이미지 → SigLIP 벡터         (시각)
  MV: AI캡션 → Qwen3 벡터          (AI 해석의 벡터 형태)
  FTS:  AI캡션+메타데이터 → FTS 키워드 (전부 다 Ctrl+F)
           ~~~~~~~ 겹침 ~~~~~~~
```

| 문제 | 설명 |
|------|------|
| MV ↔ FTS 중복 | 둘 다 같은 AI 캡션+태그를 다른 방법으로 검색 |
| MV가 메타데이터 무시 | 캡션+태그 2개 필드만 벡터화. 레이어명, 폴더, 사용자 메모 등 미활용 |
| AI가 이미지만 봄 | 폴더 구조, 레이어명 등 맥락 정보 없이 캡션 생성 |
| FTS 무차별 검색 | AI 해석과 메타데이터를 구분하지 않고 16컬럼 한꺼번에 검색 |
...
**현재 FTS와 차이**: AI가 생성한 컬럼(ai_caption, ai_tags, ai_style 등)을 **제외**하고 메타데이터 컬럼만 검색.

---

### 축 3: MV (Semantic) — AI 통합 해석 벡터
...
| 구성 요소 | 현재 | 변경 후 |
|-----------|------|---------|
| `vec_files` | SigLIP 이미지 벡터 | 그대로 (VV) |
| `vec_text` | 캡션+태그만 벡터화 | **통합 AI 해석** 벡터화 (MV) |
| `files_fts` | 16컬럼 무차별 검색 | **메타데이터 컬럼만** (FTS) |
...
| query_type | 의도 | 주력 축 |
|------------|------|---------|
| visual | 색감, 분위기, 시각적 묘사 | VV (50%) |
| keyword | 파일명, 레이어명, 폰트명 등 구체적 이름 | FTS (60%) |
| semantic | 용도, 맥락, 추상적 설명 | MV (55%) |
| balanced | 혼합 또는 분류 불확실 | MV 약간 우세 (40%) |
...
| 쿼리 | 현재 동작 | 변경 후 동작 |
|------|----------|-------------|
| "보스전 배경" | user_note에 "보스전" 있어도 MV 무반응. FTS 키워드만 매칭 | FTS: user_note "보스전" 키워드 매칭. MV: 캡션에 "boss battle arena" 의미 매칭 |
| "Shadow 레이어 있는 파일" | FTS: 16컬럼에서 "Shadow" 키워드 찾음 (AI캡션 노이즈 포함) | FTS: layer_names에서만 정확 매칭 |
| "NotoSans 폰트 사용" | FTS에서 찾지만 Tv와 혼합 순위 | FTS: used_fonts에서 정확 매칭. 깔끔한 결과 |
| "파란 톤 야경" | VV만 유효 | VV + MV(메타 맥락 포함 캡션) 이중 매칭 |
| "Characters 폴더의 검 든 캐릭터" | FTS 키워드 + Tv 부분 매칭 | FTS(folder 키워드) + MV("sword character" 의미) 정확 분리 |
| "그림자 효과 적용된 일러스트" | FTS: "그림자" 키워드만 | MV: AI가 "Shadow_Effect → 그림자 효과" 해석한 캡션에서 의미 매칭 |
...
1. **MV FTS 보조**: AI 캡션에 대한 키워드 검색을 MV에 추가 (벡터 + FTS 하이브리드)
2. **FTS 벡터화**: 메타데이터도 벡터화해서 의미 검색 추가 (예: "그림자 효과" → "Shadow_Effect")
3. **사용자 메모 실시간 반영**: 사용자가 메모/태그 수정 시 MV 캡션 재생성 없이 FTS만 즉시 업데이트
...
| 2 | SigLIP 2 NaFlex | 이미지 벡터 인코딩 (VV) | 변경 없음 |
| 3 | qwen3-embedding:0.6b | 통합 캡션 벡터 인코딩 (MV) | 변경 없음 (입력 캡션이 더 풍부해짐) |
