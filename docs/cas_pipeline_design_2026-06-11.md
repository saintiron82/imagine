...
1. **중복 내용 1회 계산**: 같은 픽셀은 클러스터 전체에서 phase당 한 번만 분석한다.
2. **모델 교체 = 자연스러운 파도**: 새 모델 버전 키의 부재가 곧 pending이고, 구 결과는 교체 완료까지 검색을 계속 서빙한다.
3. **이동·개명 내성**: 파일을 옮겨도 파생물이 따라온다 (relink 도구의 점진적 불필요화).
...
| 단계 | 내용 | 위험 | 검증 |
|---|---|---|---|
| **M0** | 선행 마감: 잡 공정성(claim 잡 단위 라운드로빈), 썸네일 잔여물 수정 | 낮음 | 단위 테스트 |
| **M1** | derivations + model_registry DDL, 결과 저장 시 **shadow write** (읽기 무변경) | 낮음 | 적립 행 검증 |
