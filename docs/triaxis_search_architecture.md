...
| 항목 | 값 |
|------|------|
| 모델 | `google/siglip2-so400m-patch16-naflex` (SigLIP 2 NaFlex) |
| 파라미터 | 4억 (So400m) |
| 벡터 차원 | **1152** (float32) |
| 특징 | NaFlex: 원본 종횡비 보존, 동적 해상도 |
...
## 축 2: Tv-axis (Text Vector) — "AI가 쓴 설명이 쿼리와 의미적으로 통하는가"
...
V-axis가 "눈으로 보는" 것이라면, Tv-axis는 "설명을 읽고 이해하는" 것.

### 모델

| 항목 | 값 |
|------|------|
| 임베딩 모델 | `qwen3-embedding:0.6b` (Ollama) |
| 벡터 차원 | **1024** (float32) |
| 캡션 생성 | `qwen3-vl:8b` (비전 모델) |
| 서버 | Ollama (`http://localhost:11434`) |
...
원본 이미지
  ...
  → qwen3-embedding:0.6b 텍스트 인코더 통과
  → 1024차원 벡터 (L2 정규화)
  → vec_text 테이블에 저장
```

**입력**: AI가 생성한 캡션 + 태그 (텍스트)
**출력**: 1024개의 숫자 배열 = 이미지 설명의 "의미적 지문"
...
## 축 3: F-axis (Full-Text Search) — "검색어가 글자 그대로 데이터에 있는가"
...
V-axis가 "눈으로 보기", Tv-axis가 "뜻을 이해하기"라면, F-axis는 **"Ctrl+F 찾기"**.
...
- **약점**: 동의어 불가. "전사"를 검색해도 "warrior"는 안 나옴 (그건 Tv-axis의 역할)
- **약점**: 오타에 취약
...
107.psd:
  ...
  Tv-axis 5등 (rank=4): 0.30 / (60 + 4 + 1) = 0.00462
  F-axis 3등 (rank=2):  0.50 / (60 + 2 + 1) = 0.00794
  ───────────────────────────────────────────
  RRF 합산 = 0.01584
```
...
| query_type | V-axis | Tv-axis | F-axis | 적용 상황 |
|------------|--------|---------|--------|-----------|
...
예: "파란 톤의 야경" → visual → V-axis에 50% 가중치
예: "건널목 상점가" → keyword → F-axis에 50% 가중치
...
이미지 파일
  │
  ├─→ 썸네일 생성 → SigLIP 2 인코더 → 1152차원 벡터 → vec_files (V-axis)
  │
  ├─→ qwen3-vl 비전 분석 → ai_caption + ai_tags 생성
  │     │
  │     ├─→ qwen3-embedding → 1024차원 벡터 → vec_text (Tv-axis)
  │     │
  │     └─→ 캡션+태그+레이어명+메모+... → files_fts 인덱스 (F-axis)
  │
...
사용자 쿼리
  ...
  │
  ├─→ V-axis:  SigLIP 2 텍스트 인코더 → vec_files 코사인 유사도 → 상위 40개
  ├─→ Tv-axis: qwen3-embedding 인코더 → vec_text 코사인 유사도 → 상위 40개
  ├─→ F-axis:  FTS5 MATCH 키워드 검색 → 상위 40개
  │
...
-- V-axis 벡터 저장 (SigLIP 2, 1152차원)
vec_files (file_id, embedding FLOAT[1152])

-- Tv-axis 벡터 저장 (Qwen3-Embedding, 1024차원)
vec_text (file_id, embedding FLOAT[1024])

-- F-axis 전문 검색 인덱스 (16컬럼)
files_fts (
    file_path, file_name,
    ai_caption, ai_tags, ai_style, ocr_text, dominant_color,
...
| 뱃지 | 색상 | 축 | 의미 (한줄 요약) |
|-------|------|-----|-----------------|
| **V 12%** | 파랑 | V-axis | 이미지 자체가 쿼리와 시각적으로 12% 유사 |
| **Tv 67%** | 보라 | Tv-axis | AI 설명이 쿼리와 의미적으로 67% 유사 |
| **T 85%** | 초록 | F-axis | 키워드가 데이터에 85% 수준으로 매칭 |
