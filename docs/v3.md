...
**작성일**: 2026-02-08
**최종 개정일**: 2026-02-08 (v3 Final — P2 T축 완료 반영)
...
| v2 | Vision → Qwen3-VL-8B, V축 → SigLIP 2, T축 → Qwen3-Embedding, DB 이중저장, 3단계 JSON Repair | 모델 리서치 반영 |
| v3 | 실제 구현 상태 반영, P0/P1 완료 확인, 미구현 항목 정리, config/styles 제거 | 문서-코드 일치화 |
| **v3.1** | P0 100% 완료 (99파일 처리), P2 T축 구현 완료, 3축 검색 검증 | P2 완료 반영 |
...
| **P2** | T축 추가 (Qwen3-Embedding-0.6B) | +10~15%p 적중률 | **완료** (99파일 T축 임베딩, 3축 RRF 검증) |
...
- [x] 인제스트 파이프라인 T축 통합 (`ingest_engine.py`)
...
- [x] 99파일 전체 T축 리인덱싱 (avg 2.23s/file)
- [x] 3축 검색 검증 (영어+한국어, T=0.73 vs V=0.05)
...
| Qwen3-Embedding-0.6B | T축 임베딩 | ~0.4GB | 항상 ✅ |
| **검색 시 합계** | | **~1.2GB** | |
| **인제스트 시 합계** | | **~7.2GB** | |
...
## 5. [P2] T축 추가 (텍스트 임베딩) — **완료**

### 5.1 개요

V축(시각 유사도)과 **독립적인** T축(텍스트 의미 유사도)을 추가.
...
backend/pipeline/ingest_engine.py    # 인제스트 시 T축 자동 생성
backend/search/sqlite_search.py      # T축 검색 + 3축 RRF 병합
...
- `ingest_engine.py` STEP 4/4에서 V축 저장 직후 T축 생성
...
```
T-axis reindex: 99/99 succeeded — total 220.5s (avg 2.23s/file)
...
3-axis Search:
  ...
  1. UHQ 4.jpg         V=0.0529 T=0.6940  ← T축이 야경 의미 포착
  2. 야경_12.psd        T=0.7284 F=0.9581  ← T+F 복합 매칭
  3. 야경_4.psd         T=0.6870 F=0.3356
  4. 야경_11.psd        V=0.0461 F=0.9472
  5. 야경_9.psd         V=0.0459 F=0.9527
```

**핵심 발견:**
- T축이 V축보다 텍스트 의미를 훨씬 정확하게 포착 (T=0.73 vs V=0.05)
- T축이 없는 파일도 V축+F축으로 보완 가능 (3축 상호보완)
...
2. **3축과의 근본적 중복** — 구조화 필드가 하는 일을 3축이 이미 더 정확하게 수행:
   - `scene_type = "alley"` → F축(FTS)이 caption/tags의 "alley"를 직접 매칭
   - `art_style = "anime"` → V축(SigLIP 1152차원)이 시각적 스타일을 벡터로 인코딩
   - `time_of_day = "night"` → T축(Qwen3-Emb 1024차원)이 "night scene" 의미를 벡터화
...
| Qwen3-Embedding-0.6B | T축 임베딩 | Apache 2.0 | ✅ 자유 |
