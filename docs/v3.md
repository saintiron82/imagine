...
| v2 | Vision → Qwen3-VL-8B, VV → SigLIP 2, MV → Qwen3-Embedding, DB 이중저장, 3단계 JSON Repair | 모델 리서치 반영 |
...
| **P1** | VV 모델 교체 (SigLIP 2 So400m) | +5~10%p 적중률 | **완료** (마이그레이션+재인덱싱 완료) |
| **P2** | MV 추가 (Qwen3-Embedding-0.6B) | +10~15%p 적중률 | **완료** (99파일 MV 임베딩, 3축 RRF 검증) |
...
- [x] 인제스트 파이프라인 MV 통합 (`ingest_engine.py`)
...
- [x] 99파일 전체 MV 리인덱싱 (avg 2.23s/file)
...
│         ──→ SigLIP 2 So400m → Vector_A (VV 1152차원)      │
...
| SigLIP 2 So400m | VV 임베딩 | ~0.8GB | 항상 |
| Qwen3-Embedding-0.6B | MV 임베딩 | ~0.4GB | 항상 ✅ |
| **검색 시 합계** | | **~1.2GB** | |
| **인제스트 시 합계** | | **~7.2GB** | |
...
## 5. [P2] MV 추가 (Meaning Vector) — **완료**

### 5.1 개요

VV(시각 유사도)와 **독립적인** MV(텍스트 의미 유사도)를 추가.
...
backend/pipeline/ingest_engine.py    # 인제스트 시 MV 자동 생성
backend/search/sqlite_search.py      # MV 검색 + 3축 RRF 병합
...
- `ingest_engine.py` STEP 4/4에서 VV 저장 직후 MV 생성
...
```
MV reindex: 99/99 succeeded — total 220.5s (avg 2.23s/file)
...
3-axis Search:
  ...
  1. UHQ 4.jpg         V=0.0529 S=0.6940  ← MV가 야경 의미 포착
  2. 야경_12.psd        S=0.7284 M=0.9581  ← MV+FTS 복합 매칭
...
**핵심 발견:**
- MV가 VV보다 텍스트 의미를 훨씬 정확하게 포착 (S=0.73 vs V=0.05)
- MV가 없는 파일도 VV+FTS로 보완 가능 (3축 상호보완)
...
- 특히 VV가 약한 스타일라이즈 이미지에서 캡션 기반 복구력 향상
...
2. **3축과의 근본적 중복** — 구조화 필드가 하는 일을 3축이 이미 더 정확하게 수행:
   - `scene_type = "alley"` → FTS가 caption/tags의 "alley"를 직접 매칭
   - `art_style = "anime"` → VV(SigLIP 1152차원)가 시각적 스타일을 벡터로 인코딩
   - `time_of_day = "night"` → MV(Qwen3-Emb 1024차원)가 "night scene" 의미를 벡터화
...
- ✅ 검색 품질은 VV+MV+FTS 3축 Auto-Weighted RRF가 전담
...
| VV 인코딩 | ~500ms (CLIP) | ~500ms (CLIP) | **~200ms (SigLIP 2)** | ~200ms |
| MV 인코딩 | - | - | - | **+~100ms** |
...
| SigLIP 2 So400m | VV 임베딩 | Apache 2.0 | ✅ 자유 |
| Qwen3-Embedding-0.6B | MV 임베딩 | Apache 2.0 | ✅ 자유 |
