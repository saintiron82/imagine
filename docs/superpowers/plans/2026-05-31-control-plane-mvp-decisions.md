...
| 항목 | 값 |
|------|-----|
| 선택 | **부분 잠금 (2026-06-02)**: Cloudflare 범위 = Edge·CDN·R2·라우팅만. Workers + D1 풀스택 제외. 백엔드 호스팅 벤더(Fly.io / Railway / 자체 VPS 등)는 별도 결정 대기. |
| 결정자 | 사용자 |
| 결정일 | 2026-06-02 (부분) |
| 근거 메모 | AskUserQuestion에서 "Edge·CDN·라우팅만 (권장)" 선택. D1 한계와 Stripe webhook 친화성 근거 채택. 백엔드 벤더는 v1 빌드 시점에 다시 결정. |

### Sub-decision: 도메인 (2026-06-02)

| 항목 | 값 |
|------|-----|
| 선택 | **`imagine.zeroechodaily.com`** (zeroechodaily.com 서브도메인) |
| 결정자 | 사용자 |
| 결정일 | 2026-06-02 |
| 근거 메모 | 비용 0, 도메인 보유 활용. 단 사용자 신뢰가 zeroechodaily 평판에 묶이는 트레이드오프 수용. 향후 자체 도메인 분리는 항상 가능. |
